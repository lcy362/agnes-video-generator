...
| R5 | [platform_features_research.md](platform_features_research.md) | 同类创作平台的通用功能设计（风格预设库 / 产物画廊 / 凭据站点分组展示） | 不在当前路线图（优先稳定性加固）；预设库的 22 语言 i18n 同步有前置门槛；画廊需先定缩略图生成时机 | 2026-09-20 | **初次归档**：按 P0-1（预设库，含修复非中文界面 `style` 预填中文默认值）/ P1（产物画廊）/ P2（凭据站点分组展示）排序；复核确认「凭据-域名绑定」多站点能力已具备（含接口与前端界面），该项降为展示层增量；明确不采纳多租户平台治理类能力（用户体系、额度、权限、审核、社区）与自由画布。复查触发条件：模板类诉求集中出现 / 非中文默认风格导致结果偏差的反馈 / 稳定性批次落地后前端有余量 |
